...
function listar(){
	tabla=$('#tbllistado').dataTable({
		...
		"order":[[2,"desc"],[3,"desc"],[0,"desc"]]//ordenar (columna, orden)
	}).DataTable();
}
...
function mostrar(id_carga){
	$.post("../ajax/productividad.php?op=mostrar",{id_carga : id_carga},
...
function desactivar(id_carga){
	bootbox.confirm("¿Esta seguro de desactivar este dato?", function(result){
		if (result) {
			$.post("../ajax/productividad.php?op=desactivar", {id_carga : id_carga}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}
...
function activar(id_carga){
	bootbox.confirm("¿Esta seguro de activar este dato?" , function(result){
		if (result) {
			$.post("../ajax/productividad.php?op=activar", {id_carga : id_carga}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}
